...
Herdr can be selected explicitly or by runtime auto-detection: its terminal lifecycle is separate from workspace isolation, so workers use the shared checkout by default and use Treehouse only when `config/workspace-isolation=worktree`; [`herdr-backend.md`](herdr-backend.md) owns current setup, CI coverage, and safety limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#herdr) owns active empirical evidence.
...
Zellij is experimental and selected only explicitly: its terminal lifecycle is separate from workspace isolation, so workers use the shared checkout by default and use Treehouse only when `config/workspace-isolation=worktree`; [`zellij-backend.md`](zellij-backend.md) owns current setup and limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#zellij) owns active empirical evidence.
...
cmux is experimental, GUI-first, macOS-only, and can be selected explicitly or by runtime auto-detection from its primary `CMUX_WORKSPACE_ID` marker plus documented fallback signals: its terminal lifecycle is separate from workspace isolation, so workers use the shared checkout by default and use Treehouse only when `config/workspace-isolation=worktree`; [`cmux-backend.md`](cmux-backend.md) owns current setup and limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#cmux) owns active source and live evidence.
...
Ordinary ship and scout workers run in the project's existing shared checkout by default.
`config/workspace-isolation=worktree` opts a home back into the inherited FirstMate model where [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, and cmux tasks, while Orca creates its own worktrees for `backend=orca`.
The [`fm-spawn.sh` header](../bin/fm-spawn.sh) owns ship/scout workspace selection, including shared-checkout defaulting, worktree isolation, and fresh-base refusal rules for the explicit worktree mode.
Portable regressions live in [`tests/fm-workspace-isolation.test.sh`](../tests/fm-workspace-isolation.test.sh) for mode selection and shared-checkout cleanup, [`tests/fm-spawn-pool-base-freshen.test.sh`](../tests/fm-spawn-pool-base-freshen.test.sh) for worktree-mode spawn isolation and base freshness, and [`tests/fm-control-relaunch.test.sh`](../tests/fm-control-relaunch.test.sh) for preserving the recorded copy on relaunch.
Shared-checkout mode is intentionally not isolation: concurrent workers can edit the same files, so operators should serialize overlapping work or opt into worktree mode.
Cleanup in shared-checkout mode closes the recorded worker endpoint and task records without resetting, deleting, branching, returning, or otherwise treating the project checkout as disposable.
...
In worktree mode, ship briefs tell the crewmate to verify `pwd -P` and `git rev-parse --show-toplevel` before creating `fm/<id>`, then stop with a blocked status if it landed in the primary checkout.
That placement is proven only at launch, so `bin/fm-spawn.sh` also exports the task id as `FM_TASK_ID` into worktree-mode ship and scout panes, and `bin/fm-test-run.sh` refuses to execute the behavior suite from the primary checkout while that marker is set; the runner's header owns the predicate and [`tests/fm-test-run.test.sh`](../tests/fm-test-run.test.sh) pins it.
In shared-checkout mode, ship briefs instead say to inspect `git status --short` and not create a branch or Git worktree.
